      ...
      <div className="clay-card p-8 md:p-12 min-h-[400px]">
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div 
              ...
              className="space-y-6"
            >
              <h2 className="text-3xl font-bold mb-6">Traveler Details</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">First Name</label>
                  <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary focus:bg-white transition-colors" placeholder="John" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Last Name</label>
                  <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary focus:bg-white transition-colors" placeholder="Doe" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Email Address</label>
                  <input type="email" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary focus:bg-white transition-colors" placeholder="john@example.com" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Special Requests</label>
                  <textarea className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:border-primary focus:bg-white transition-colors h-24 resize-none" placeholder="Dietary requirements, accessibility needs, etc." />
                </div>
              </div>
            </motion.div>
          )}
          ...
          {step === 3 && (
            ...
            >
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6 mb-8 flex justify-between items-center">
                <div>
                  <p className="text-sm text-gray-500 font-bold mb-1">Trioo Wallet Balance</p>
                  <p className="text-2xl font-bold text-gray-900">₹82,000</p>
                </div>
                <button className="bg-primary text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm">
                  Use Wallet
                </button>
              </div>

              <div className="space-y-4">
                <p className="font-bold text-gray-700">Other Payment Methods</p>
        ...
        <div className="mt-12 flex justify-between items-center pt-6 border-t border-gray-100">
          ...
          <button 
            ...
            className="clay-btn-primary px-8 py-4 flex items-center gap-2 text-lg"
          >
            {step === 3 ? 'Confirm Booking' : 'Continue'} <ArrowRight className="w-5 h-5" />
          </button>
        </div>
      </div>
    </div>
